...
interface LeaderboardProps {
  ...
  onClose: () => void;
  onTransformCta?: () => void;
  /** Hide premium pitch for already-subscribed users */
  isPremium?: boolean;
}
...
export default function Leaderboard({
  ...
  useEffect(() => {
    ...
    fetchLeaderboard();
    // Persistent pitch: open when leaderboard opens for non-premium users
    if (!isPremium) {
      const t = window.setTimeout(() => setShowTransformPitch(true), 350);
  ...
  const openPitch = () => {
    if (!isPremium) setShowTransformPitch(true);
  };
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        ...
        >
          {/* Header */}
          <div className="flex-shrink-0 z-20 bg-black border-b border-white/10">
            <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-4 sm:pt-6 pb-4">
                ...
                <div className="flex items-center gap-3">
                  {!isPremium && (
                    <button
                      type="button"
                      onClick={openPitch}
                      className="hidden sm:inline-flex px-3 py-1.5 text-xs font-bold rounded-lg bg-[#22c55e] text-black hover:bg-white transition-colors"
                    >
                      Go Premium
                    </button>
                  )}
              ...
              <div className="text-center">
                ...
                <p className="text-zinc-500 text-sm">
                  Top faces ranked by analysis score
                  {!isPremium && (
                    <span className="text-white/30">
                      {' '}
                      · Premium members marked with ★
                    </span>
                  )}
                </p>
              </div>
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-4">
              {!isPremium && (
                <button
                  type="button"
                  onClick={openPitch}
                  className="w-full mb-5 rounded-xl border border-[#22c55e]/35 bg-[#22c55e]/10 px-4 py-3 text-left flex items-center gap-3 hover:bg-[#22c55e]/15 transition-colors"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-white">
                      Want to climb? Unlock your protocol
                    </p>
                    <p className="text-[11px] text-white/45 mt-0.5 truncate">
                      {SUBSCRIPTION_PRICING.trialDays}-day free trial · then{' '}
                      {SUBSCRIPTION_PRICING.yearly.label}
                    </p>
                  </div>
                  <span className="text-[#22c55e] text-xs font-bold shrink-0">Try free</span>
                </button>
              )}
                  ...
                  {error.includes('authentication') || error.includes('token') ? (
                    <>
                      ...
                      <p className="text-zinc-600 text-sm">
                        Sign in to view and compete on the leaderboard
                      </p>
                    </>
                  ) : (
                    <>
                ...
                <div className="text-center py-20 px-4">
                  ...
                  {!isPremium && (
                    <button
                      type="button"
                      onClick={goPremium}
                      className="inline-block px-5 py-2.5 bg-[#22c55e] hover:bg-white text-black text-xs font-bold uppercase tracking-wide rounded-xl transition-colors"
                    >
                      Transform with Premium
                    </button>
                  )}
                </div>
              ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-5 pt-1">
                  {entries.map((entry, index) => (
                    ...
                    >
                      ...
                      <div
                        className={`aspect-[3/4] rounded-2xl overflow-hidden bg-zinc-900 border transition-all duration-300 ${
                          index === 0
                            ? 'border-[#22c55e]/50 shadow-lg shadow-green-500/10'
                            : index === 1
                              ? 'border-slate-400/30'
                              : index === 2
                                ? 'border-zinc-500/30'
          ...
          {/* Persistent premium dock — always visible for non-premium */}
          {!isPremium && (
            <div className="absolute bottom-0 inset-x-0 z-30 pointer-events-none">
              <div className="pointer-events-auto border-t border-[#22c55e]/30 bg-black/95 backdrop-blur-md px-3 sm:px-4 py-3 safe-area-pb">
                <div className="max-w-6xl mx-auto flex items-center gap-3">
                  ...
                  >
                    ...
                    <p className="text-[11px] text-white/45 truncate">
                      {SUBSCRIPTION_PRICING.trialDays}-day free trial ·{' '}
                      {SUBSCRIPTION_PRICING.yearly.label} or {SUBSCRIPTION_PRICING.monthly.label}
                    </p>
                  </button>
                  <button
                    type="button"
                    onClick={goPremium}
                    className="shrink-0 px-4 py-2.5 rounded-xl bg-[#22c55e] hover:bg-white text-black text-xs sm:text-sm font-bold transition-colors"
                  >
                    Start trial
                  </button>
                </div>
          ...
          <TransformPitchModal
            isOpen={showTransformPitch && !isPremium}
            onClose={() => setShowTransformPitch(false)}
            onCta={goPremium}
            dismissLabel="Keep browsing leaderboard"
          />
        </motion.div>
      )}
    </AnimatePresence>
